...
export class TronProvider extends BaseProvider {
    ...
    constructor(opts: TronProviderOptions = {}) {
        ...
        this._fullNode = opts.fullNode || process.env.TRON_RPC_URL || 'https://api.trongrid.io';
        this._solidityNode = opts.solidityNode || process.env.TRON_RPC_URL || 'https://api.trongrid.io';
        this._eventServer = opts.eventServer || process.env.TRON_RPC_URL || 'https://api.trongrid.io';
    ...
    async sendTransaction(toAddress: string, amount: number): Promise<any> {
        ...
        const tradeobj = await this.tronWeb.transactionBuilder.sendTrx(toAddress, amount, this.address);
